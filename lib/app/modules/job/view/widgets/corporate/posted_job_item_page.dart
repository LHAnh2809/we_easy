...
import 'detailed_work_posted_page.dart';

class PostedJobItemPage extends GetView<JobController> {
  final PostedJobModel model;
  final String time;
  const PostedJobItemPage({Key? key, required this.model, required this.time})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ButtonWidget(
      onTap: () {
        controller.getUserUT(model.postId.toString(), model.postId.toString()).then(
            (value) => Get.to(() => DetailedWorkPostedPage(model: model)));
      },
      widget: Column(
        children: [
          Container(
