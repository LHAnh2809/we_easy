...
import 'package:we_easy/app/data/model/all_job_models/all_job_model.dart';
...
import 'detailed_work_page.dart';

class WorkItemWidget extends GetView<HomeController> {
  final AllJobModel model;
  final String time;
  const WorkItemWidget({Key? key, required this.model, required this.time})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ButtonWidget(
      onTap: () {
        controller.getJob(model.postId.toString()).then(
              (value) => controller.getPostJobALL(model.idCty.toString()).then(
                    (value) => Get.to(
                      () => DetailedWorkPage(model: model),
                    ),
                  ),
            );
      },
